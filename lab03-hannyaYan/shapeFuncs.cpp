...
double distanceBetween(Point p, Point q) {
  ...
  double result = 0;
  result = sqrt( pow((q.x - p.x), 2) + pow((q.y - p.y), 2) );
  return result;
}
...
bool boxesApproxEqual(Box b1, Box b2, double tolerance) {
  ...
  bool pointSame, widthSame, heightSame;
  pointSame = pointsApproxEqual(b1.ul, b2.ul, tolerance);
  widthSame = (approxEqual(b1.width, b2.width) < tolerance);
  heightSame = (approxEqual(b1.height, b2.height) < tolerance);
  if (pointSame && widthSame && heightSame)
	  return true;
  else
	  return false;
}
